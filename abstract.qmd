...
ggplot(plot_summary, aes(x = group, y = mean)) +
  ...
  labs(
    title = "Abbildung: Mittelwertvergleich Confidence (Baseline vs. KI)",
    x = "Bedingung",
...
time_on_task_means <- data_combined %>%
  filter(system %in% c("E", "R")) %>%  # Nur KI-Systeme
  group_by(participantId) %>%                     # Gruppieren nach Nutzer
  summarise(mean_confidence_baseline = mean(time_on_task, na.rm = TRUE)) 

summary(time_on_task_means)


aiknowledge_per_user <- data_combined %>%
  select(aiknowledge, participantId)

summary(aiknowledge_per_user)

# Join mit Zeitdaten
combined_with_id <- aiknowledge_per_user %>%
  full_join(time_on_task_means, by = "participantId")

combined <- combined_with_id %>%
  select(-participantId)

# Korrelation berechnen
cor.test(combined$mean_time_on_task, combined$aiknowledge, method = "pearson")
